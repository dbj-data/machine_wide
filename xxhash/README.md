> dbj@dbj.org Note: For full repo please go to its GitHub location. Here is just a source, used in Windows only builds. For building we use only Visual Studio. Also the CLI app ha been moved to the subfolder `cli`. We use this by including `xxh3.h`. As for every other "MACHINE_WIDE" stuff, if there are `lib`'s or `dll`'s they are in `~\MACHINE_WIDE\lib`. Please add it to your `/LIBPATH`
